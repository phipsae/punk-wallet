...
export default function TokenDetailedDisplay({ tokenSettingsHelper, token, tokenCoreDisplay, network, setItemDetailed }) {
                    ...
                    {tokenSettingsHelper.isCustomItem(token) && 
                        <>
                            ...
                            <div style={{ display: "flex", alignItems:"center", justifyContent:"center", fontSize:"16px"}} >
                                {tokenSettingsHelper.isItemsTheSame(token, tokenSettingsHelper.getSelectedItem()) && <div>Cannot remove the selected token!</div>}
                            </div>
                        </>
                    }
                </>
